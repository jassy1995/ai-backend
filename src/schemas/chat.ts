...
export const conversationSchema = yup.object({
  body: yup.object({
    ...
    userId: yup.string().required("userId is required! Add 'userId' in the request body"),
  }),
});
export const messageSchemaParam = yup.object({
  params: yup.object({
    chatId: yup
      .string()
      .required("chatId is required! Add 'chatId' in params"),
  }),
});
...
export interface UpdateChatBody extends yup.InferType<typeof updateChatSchema> {}
